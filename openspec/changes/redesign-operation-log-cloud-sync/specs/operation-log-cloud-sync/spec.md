...
The system SHALL treat a Yamibo POST response as an acknowledgement hint only. It SHALL mark operations acknowledged only after authoritative reload verifies the expected journal fingerprint and exact operation ids.

#### Scenario: POST succeeds and reload matches
- **WHEN** a journal POST receives a success prompt and authoritative reload contains the exact expected operations and fingerprint
- **THEN** those operations become acknowledged

#### Scenario: POST result is unknown
- **WHEN** the request times out after the server may have accepted it
- **THEN** operations remain pending or published-unverified and the next run reloads before retrying

#### Scenario: Reload differs after POST
- **WHEN** authoritative reload does not contain the expected journal version
- **THEN** no operation is acknowledged and the engine rebases or retries within its bounded policy

### Requirement: Bounded convergence cycle
The system SHALL serialize manual and automatic sync through one durable lease and execute pull, validate, reduce, apply, publish-own-journal, verify, and pull-again phases. It SHALL stop at a fixed point or persist pending work for bounded retry without force upload.
...
- **THEN** the pull-again phase observes and reduces that operation before declaring a fixed point
...
The system SHALL construct uniquely identified immutable checkpoint blogs from a deterministic `BackupModels` state projection and causal vector. It SHALL prune an operation only after the checkpoint reload verifies and every device active within the last 90 days acknowledges that exact checkpoint and covering vector.

#### Scenario: Checkpoint candidate is written
- **WHEN** a device creates a checkpoint candidate
- **THEN** no journal operation is pruned until authoritative reload and active-device acknowledgements complete
