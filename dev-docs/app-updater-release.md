...
The app checks for updates from the GitHub repository `lmc2007/yamibo-app`. Because GitHub direct access can be slow, the client prefers the verified third-party GitHub acceleration mirror first and falls back to GitHub direct only when the mirror is unavailable:

1. `https://ghproxy.net/https://raw.githubusercontent.com/lmc2007/yamibo-app/update-release/update/stable.json`
2. `https://raw.githubusercontent.com/lmc2007/yamibo-app/update-release/update/stable.json` (GitHub direct)

Both endpoints serve the same GitHub `update-release` feed. `gh-proxy.com` is not used for update checks — it only accelerates APK downloads. Add or remove check mirrors only after verifying them with `tools/check-update-mirrors.ps1`.
...
The update feed lives in the GitHub repository `lmc2007/yamibo-app` (`update-release` branch). The client reaches it through the verified third-party GitHub acceleration mirror; the active check list is `DefaultAppUpdateRepository.sources`:

- `ghproxy.net`

`gh-proxy.com` is only used to accelerate APK downloads and is not part of the update check sources.
`ghfast.top`, `gh.llkk.cc`, and `gh.ddlc.top` were removed after verification because they timed out or returned errors during update checks.
