...
internal data class AppSyncLocalPruneResult(val removedRows: Int, val removedPayloadBytes: Long)
...
internal class AppSyncCanonicalLocalPruner(private val db: Database,
    ...
    fun prune(verified: AppSyncVerifiedCanonicalCheckpoint, now: Long, maximumRows: Int = 128): AppSyncLocalPruneResult =
        db.transactionWithResult {
            ...
            // Active recovery can still depend on acknowledged source rows during exact activation.
            val recovery = operations.getRecoverySessionByAccount(account).executeAsOneOrNull()
            if (recovery != null && recovery.phase != "COMPLETED") return@transactionWithResult AppSyncLocalPruneResult(0, 0)
            var removed = 0
            var bytes = 0L
            ...
            AppSyncLocalPruneResult(removed, bytes)
        }
