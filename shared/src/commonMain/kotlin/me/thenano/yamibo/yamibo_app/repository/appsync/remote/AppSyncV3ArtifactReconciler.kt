...
internal class AppSyncV3ArtifactReconciler(
    ...
    private suspend fun scan(title: String, sha256: String): AppSyncV3ArtifactDiscovery {
        ...
        for (index in 1..maxPages) {
            ...
            if (page.blogClasses.none { it.id == classId && it.name == className }) return AppSyncV3ArtifactDiscovery.Unknown
            val nav = page.pageNav
            if (nav?.currentPage != null && nav.currentPage != index) return AppSyncV3ArtifactDiscovery.Unknown
            val total = nav?.totalPages
            if (total != null) {
                if (total < index || total > maxPages || (expectedTotal != null && expectedTotal != total))
                    return AppSyncV3ArtifactDiscovery.Unknown
                expectedTotal = total
            }
            ...
            if (nav?.nextUrl != null && next == null) return AppSyncV3ArtifactDiscovery.Unknown
            if (next != null && (next != index + 1 || (total != null && next > total))) return AppSyncV3ArtifactDiscovery.Unknown
            if (next == null && (expectedTotal == null || index == expectedTotal)) {
                completed = true
                break
            }
        }
        if (!completed) return AppSyncV3ArtifactDiscovery.Unknown
